...
\end{DoxyItemize}\hypertarget{index_autotoc_md3}{}\doxysubsection{\texorpdfstring{Notes on $<$tt$>$bpy.\+types.\+Enum\+Property$<$/tt$>$}{Notes on <tt>bpy.types.EnumProperty</tt>}}\label{index_autotoc_md3}
When defining enum entries, you {\bfseries{must}} specify a number for it. The number are unique value used as the identifier for the entry (stored in file data). If you do not do this, the original .blend file will break (by using wrong or invalid value) when entries are added/reordered/renamed. The following is a {\bfseries{wrong}} example\+:
...
\end{DoxyCode}
\hypertarget{index_autotoc_md4}{}\doxysection{\texorpdfstring{Troubleshooting}{Troubleshooting}}\label{index_autotoc_md4}

\begin{DoxyItemize}
\item Exported scene is different or missing some data.
\begin{DoxyItemize}
\item If you are exporting via {\ttfamily File -\/\texorpdfstring{$>$}{>} Export -\/\texorpdfstring{$>$}{>} Photon Scene (.p2)}, try using the {\ttfamily Render to .p2} render engine to export instead. Unfortunately Blender currently does not support getting a depsgraph with `depsgraph.\+mode == \textquotesingle{}RENDER'{\ttfamily (except the one passed to}bpy.\+types.\+Render\+Engine{\ttfamily by Blender). This will cause the exporter to export in}VIEWPORT{\ttfamily mode, and many settings such as subdivision modifiers will not apply their}RENDER{\ttfamily mode settings. The exporter still attempts to enhance this part by trying to automatically adjust object settings to match the one used for final rendering (or what the user specifies), but chances are that some parts are still missing/different. The}Render to .p2{\ttfamily render engine export scenes in}RENDER\`{} mode by default. See \href{https://devtalk.blender.org/t/get-render-dependency-graph/12164}{\texttt{ Get "{}Render"{} dependency graph}} for a related discussion.
\end{DoxyItemize}
...
\end{DoxyItemize}
\end{DoxyItemize}\hypertarget{index_autotoc_md5}{}\doxysection{\texorpdfstring{Known Issues}{Known Issues}}\label{index_autotoc_md5}
\hypertarget{index_autotoc_md6}{}\doxysubsection{\texorpdfstring{Blender 3.\+6.\+5}{Blender 3.6.5}}\label{index_autotoc_md6}
